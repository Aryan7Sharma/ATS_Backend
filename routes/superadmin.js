...
router.post("/updatesitestatus", superadminmainController.updateSiteStatus);
...
router.post("/rep/empConsolidatedAttendanceReport",
    [
        check('start_date').exists().withMessage('Start Date is required').isLength({ min: 10 }).withMessage('Invalid Start Date'),
        check('end_date').exists().withMessage('End Date is required').isLength({ min: 10 }).withMessage('Invalid End Date'),
    ], validateBody, superadminreportController.getEmployeesConsolidatedAttendanceReport);


// working_days
router.get("/getCurrYearWorkingDays", superadminmainController.getCurrYearWorkingDays);
router.post("/updateWorkingDays", superadminmainController.updateWorkingDays);
