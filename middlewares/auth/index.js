...
const verifyUser = async (req, res, next) => {
    try {
        ...
        if (!user || user?.emp_status!=1) {return res.status(401).json({ status: env.s401, msg: 'You are not authorized user to access this!' });};
        req.user = user;
        next();
    } catch (error) {
...
const verifyAdmin = async (req, res, next) => {
    try {
        ...
        if(!decodetoken?.id){return res.status(401).json({ status: env.s401, msg: 'Decoding Token Failed!' })};
        const user = await loginsModel.findByPk(decodetoken?.id);
        if (!user || user?.emp_status!=1 || user?.user_type!==2) {return res.status(401).json({ status: env.s401, msg: 'You are not authorized user to access this!' });};
        req.user = user;
        next();
    } catch (error) {
        //console.log("error", error)
...
const verifySuperAdmin = async (req, res, next) => {
    try {
        ...
        if(!decodetoken?.id){return res.status(401).json({ status: env.s401, msg: 'Decoding Token Failed!' })};
        const user = await loginsModel.findByPk(decodetoken?.id);
        if (!user || user?.emp_status!=1 || user?.user_type!==1) {return res.status(401).json({ status: env.s401, msg: 'You are not authorized user to access this!' });};
        req.user = user;
        next();
    } catch (error) {
        //console.log("error", error)
